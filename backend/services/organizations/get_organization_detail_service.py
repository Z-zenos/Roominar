...
@cached_response(
    cache_key=CacheKeys.ORG_PROFILE,
    ttl=600,  # 10 minutes
    include_user=True,
    include_params=True,
)
def get_organization_detail(db: Session, user: User, organization_slug: str):
    """Get organization detail with events and follower information"""

    with transaction_scope() as session:
        # Get organization tags subquery
        OrganizationTag = (
            select(
                Organization.id,
                func.json_agg(
        ...
        # Get organization event and follow count subquery
        ...
        query_params = SearchEventsQueryParams().create(
            organization_id=organization["id"]
        )
        events, total_public_events = events_service.search_events(
            db,
            user,
            query_params,
        )
