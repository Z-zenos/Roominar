...
def create_application_checkout_session(
    db: Session,
    current_user: User,
    create_application_request: CreateApplicationRequest,
):
    """Create Stripe checkout session for event application"""
    ...
    with transaction_scope() as session:
        try:
            ...
            result = applications_service.validate_application_tickets(
                session, current_user.id, create_application_request, False
            )
            result["event"]
            ...
            raise BadRequestException(
                ErrorCode.ERR_STRIPE_ERROR, f"{ErrorMessage.ERR_STRIPE_ERROR}: {str(e)}"
            )
