    ...
    export class EventsApiRequestFactory extends BaseAPIRequestFactory {
        ...
        */
        ...
        /**
            ...
            * @param slug
        */
        public async getEventIdBySlug(slug: string, _options?: Configuration): Promise<RequestContext> {
            const _config = _options || this.configuration;

                    // verify required parameter 'slug' is not null or undefined
                    if (slug === null || slug === undefined) {
                    throw new RequiredError("EventsApi", "getEventIdBySlug", "slug");
                    }


            // Path Params
            const localVarPath = '/api/v1/events/{slug}/id'
                .replace('{' + 'slug' + '}', encodeURIComponent(String(slug)));
        ...
        */
        ...
        export class EventsApiResponseProcessor {
            ...
            * to the expected objects
            *
            * @params response Response returned by the server for a request to getEventIdBySlug
            * @throws ApiException if the response code was not in [200, 299]
            */
            public async getEventIdBySlugWithHttpInfo(response: ResponseContext): Promise<HttpInfo<number >> {
            const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
                if (isCodeInRange("200", response.httpStatusCode)) {
                        const body: number = ObjectSerializer.deserialize(
                        ObjectSerializer.parse(await response.body.text(), contentType),
