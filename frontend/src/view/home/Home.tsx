...
import { MdCategory, MdKeyboardDoubleArrowRight, MdOutlineExplore } from 'react-icons/md';
...
const HeadingGroup = ({ heading, subheading, className }: HeadingGroupProps) => {
  return (
...
const rankingEvents: Rank[] = [
  ...
  {
    rank: 5,
    title: 'みんなの考えた最強のデータ基盤アーキテクチャ2024前半おまとめ拡大版SP！',
  },
];
  ...
  return (
    <div>
      ...
      <section className='text-center flex items-center justify-center flex-col py-[80px]'>
        <h2 className={clsx('flex justify-center items-center gap-5 italic text-lg')}>
          <span>Explore</span>
        ...
        <h1 className='text-hg my-5 font-semibold'>
          Web(<span className='text-gradient'>Sem</span>)inar & <span className='text-gradient'>E</span>vent 2024 🎉
        </h1>
        <p className='text-primary font-semibold mb-8'>
          Search site for business seminars focusing on digital and AI utilization
        </p>
        <span className='border-t-1 border-gray-600 border-b-1 py-1 px-4'>🚀 | YOU&apos;VE GOT PLANS?</span>
        <Input
          className='max-w-[500px] mt-5'
          placeholder='Search any event you want.'
          startContent={<CiSearch className='w-5 h-5 pointer-events-none flex-shrink-0' />}
      ...
      <section className='px-[15%] pt-[20px]'>
        <div className='flex justify-between items-end gap-2 flex-wrap-reverse'>
          <div className={clsx('mt-10', width > 1200 ? 'w-[40%]' : 'w-full mx-auto')}>
            <Swiper
              key={2}
              autoplay={{
                delay: 3500,
          ...
          <div className='h-full'>
            ...
            <div className='grid grid-cols-6 gap-4 '>
              {['music', 'sport', 'technology', 'business', 'golf', 'esport', 'life & style', 'AI', 'Logistics'].map(
                (c) => (
      ...
      <section className='px-[15%] mb-7'>
        <div
          className={clsx('flex flex-wrap justify-between items-start gap-10', width > 1200 ? 'flex-row' : 'flex-col')}
        >
          <div className={clsx(width > 1200 ? 'w-[70%]' : 'w-full')}>
            ...
            <div className={clsx('grid items-center gap-4 mt-6 ', width > 1200 ? ' grid-cols-3' : ' grid-cols-2 ')}>
            ...
            <div className='flex justify-between gap-2 items-center bg-info-sub mt-8 rounded-md px-10 py-8'>
              <div>
                <h3 className='font-semibold text-xm text-info-main'>Start an event with Roominar</h3>
                <p className='font-light opacity-75'>
                  Anyone can create an event page for free by creating a group. Why not publish an event for sharing
                  information and interacting with others on Roominar?
                </p>
              </div>
              <Button
      ...
      <section className='pb-[40px] px-[15%]'>
        <div>
          ...
          <h3 className='text-xm text-gray-600 font-light'>Inspiring insights from visionary Speaker.</h3>
